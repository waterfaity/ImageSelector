...
public class ViewScaleTool {
    ...
    public ViewScaleTool(View view) {
        this.view = view;
        //手势
        gestureDetector = new GestureDetector(view.getContext(), getGestureListener());
        //缩放手势
        scaleGestureDetector = new ScaleGestureDetector(view.getContext(), getScaleGestureListener());
    }
...
//                Log.i(TAG, "onFling: " + velocityX + "," + velocityY + " -- " + e1.getX() + "," + e1.getY() + "  --  " + e2.getX() + "," + e2.getY());
    ...
    private Matrix getViewMatrix() {
        if (view instanceof ImageView) return ((ImageView) view).getImageMatrix();
        else return view.getMatrix();
    }
...
//                Log.i(TAG, "onAnimationUpdate: " + targetScaleTemp + "   " + targetScaleTemp + "   " + x + "   " + y);
    ...
    private boolean move(int x, int y) {
        ...
        if ((isDrag || isFling) && !isZoom) {
            ...
            if (matrixRectF.width() < view.getWidth()) {
                dx = 0;
            } else {
                if (dx > 0) {
                    //右移
                    ...
                    //左移
                    if (matrixRectF.right + dx < view.getWidth()) {
                        dx = view.getWidth() - matrixRectF.right;
                        if (isHorMove)
                            move = false;
                    }
                }
            }
            if (matrixRectF.height() < view.getHeight()) {
                dy = 0;
            } else {
                if (dy > 0) {
                    //下移
                    ...
                    //上移
                    if (matrixRectF.bottom + dy < view.getHeight()) {
                        dy = view.getHeight() - matrixRectF.bottom;
                        if (!isHorMove)
                            move = false;
                    }
                }
...
//        Log.e(TAG, "deltaX = " + deltaX + " , deltaY = " + deltaY);
        ...
        getViewMatrix().postTranslate(deltaX, deltaY);
    ...
    private float getScale() {
        getViewMatrix().getValues(matrixValues);
