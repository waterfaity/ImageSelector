...
public class SelectPresenter implements SelectPresenterListener {
    ...
    public SelectPresenter(ImageSelectActivity activity) {
        mActivity = activity;
        mView = activity;
        mModel = new SelectModel(this, mActivity);
    }

    public void getExtra(Intent intent) {
        mDeep = intent.getIntExtra(ImageSelectActivity.SEARCH_DEEP, mDeep);
        mIgnorePaths = intent.getStringArrayListExtra(ImageSelectActivity.IGNORE_PATHS);
        mSearchPaths = intent.getStringArrayListExtra(ImageSelectActivity.SEARCH_PATHS);
    }
    ...
    public void onGetFoldersSuccess(ArrayList<SearchFolderBean> folders) {
        if (!mView.isDestroy()){
            ...
            if (folders == null || folders.size() == 0) {
                Toast.makeText(mActivity, "没有发现图片", Toast.LENGTH_SHORT).show();
                return;
            }
